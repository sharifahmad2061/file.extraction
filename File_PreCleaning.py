...
class Handler(FileSystemEventHandler): 
    ...
    def on_any_event(event): 
        if event.is_directory: 
            ...
            file_extension_original = file_extension_str.lower()  # Important otherwise upper case extensions would not be handeled correctly
            file_name_original      = os.path.basename(file_path)
            file_size_kb_original   = file_stats.st_size / 1000
            meta_accessed_original  = datetime.datetime.utcfromtimestamp(file_stats.st_atime).strftime('%Y-%m-%dT%H:%M:%SZ')
            meta_created_original   = datetime.datetime.utcfromtimestamp(file_stats.st_ctime).strftime('%Y-%m-%dT%H:%M:%SZ')
            meta_modified_original  = datetime.datetime.utcfromtimestamp(file_stats.st_mtime).strftime('%Y-%m-%dT%H:%M:%SZ')

            print(f'File name:                      {file_name_original}')
            print(f'File extension:                 {file_extension_original}')
            print(f'File size original in KB:       {file_stats.st_size / 1024}')
            ...
            try:
                ...
                print(f'MD5 hash original:              {hasher_md5.hexdigest()}')    
                ...
                if not result_check[0]:
                    ...
                    if file_extension_original in archive_extensions_permitted:
                        ...
                        # Extract archive
                        try:
                                patoolib.extract_archive(file_path, outdir=extraction_path)
                        ...
                        shutil.copy2(file_path, path_for_originals + '/archives/' + file_uuid + file_extension_original, follow_symlinks=True)
                    ...
                    if file_extension_original in excel_extensions_permitted:
                            ...
                            print(f'New path after conversion:      {path_after_conversion}')

                            # Obtain new file size
                            size_after_conversion = os.path.getsize(path_after_conversion) / 1000
                            print(f'New file size after conversion: {size_after_conversion}')
                            ...
                            try:
                                ...
                                with open(path_after_conversion, 'rb') as afile:
                                    ...
                                    print(f'MD5 hash after conversion:      {md5_converted}')
                            ...
                            os.rename(file_path, path_for_originals + '/excel/' + file_uuid + file_extension_original)
